...
async def get_month_availability(location: str):
   conn = get_db()
   cursor = conn.cursor()
   cursor.execute("SELECT res_date, capacity FROM custom_capacities WHERE location = ?", (location,))
   custom_caps = dict(cursor.fetchall())
   cursor.execute("SELECT res_date, COUNT(*) FROM reservations WHERE location = ? GROUP BY res_date", (location,))
...
async def make_reservation(
   ...
   mudurluk: str = Form(...),
   location: str = Form(...),
   dates_json: str = Form(...)  # Örn: ["2026-09-01", "2026-09-08"]
):
   try:
       selected_dates = json.loads(dates_json)
   except:
       return JSONResponse(status_code=400, content={"message": "Geçersiz tarih formatı!"})
   if not selected_dates:
       return JSONResponse(status_code=400, content={"message": "Lütfen en az bir tarih seçiniz!"})
   # Haftalık 2 gün kuralı sunucu taraflı kontrolü
   for week_name, days in SEPTEMBER_2026_WEEKS.items():
       count_in_week = sum(1 for d in selected_dates if d in days)
       if count_in_week > 2:
           return JSONResponse(status_code=400, content={"message": f"{week_name} içerisinde 2 günden fazla seçim yapamazsınız!"})
   conn = get_db()
   cursor = conn.cursor()
   ...
   if errors and not valid_dates:
       conn.close()
       return JSONResponse(status_code=400, content={"message": " <br>".join(errors)})
...
async def get_all_reservations(password: str):
   ...
   conn.close()
   reservations = [
       {"id": r[0], "name": r[1], "baskanlik": r[2], "mudurluk": r[3], "location": r[4], "res_date": r[5], "created_at": r[6]}
       for r in rows
   ]
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
           ...
           function toggleDateSelection(checkbox) {
               ...
               if (checkbox.checked) {
                   // İlgili haftadaki seçili gün sayısı kontrolü
