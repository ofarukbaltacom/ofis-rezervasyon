...
def normalize_text(text):
 ...
 tr_map = str.maketrans("İĞÜŞÇÖığıüşçö", "IGUSCOigusco")
...
def get_reservation_info_by_name(sender_name):
 ...
 for row in rows:
   ...
   sender_parts = [p for p in normalized_sender.split() if len(p) > 1]
   if sender_parts and all(part in normalized_db_name for part in sender_parts):
   ...
   try:
     ...
     for message in messages:
       ...
       if is_office_reservation_request(subject, body):
         ...
         if reservations:
           pnr = reservations[0][0]
           real_name = reservations[0][1]
           dates_text = "<br>".join(
               [f"• {r[3]} ({r[2]})" for r in reservations]
           )
           reply = message.Reply()
           reply.Subject = (
...
<p>Uydu ofis rezervasyon iptal veya değişiklik talebiniz incelenmiştir. Sistemimizde Eylül 2026 dönemi için aktif rezervasyon bilgileriniz aşağıdadır:</p>
<p>🔑 <b>PNR Kodunuz:</b> <span style="color: red; font-weight: bold; font-family: monospace;">{pnr}</span></p>
<p><b>Rezerve Ettiğiniz Günler:</b><br>{dates_text}</p>
<br>
<p>İşlemlerinizi (gün değiştirme veya iptal etme) dışarıdan desteğe gerek kalmadan, yukarıdaki <b>PNR Kodunuzu</b> ve sicilinizi kullanarak <a href="http://uyduofisrezervasyon.com">Uydu Ofis Rezervasyon Portalı</a> üzerinden kendiniz kolayca yapabilirsiniz.</p>
<br>
<p>İyi çalışmalar dileriz.<br><b>Organizasyonel Gelişim Müdürlüğü</b></p>
                   """
               + reply.HTMLBody
           )
           reply.Send()
