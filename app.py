...
# ==============================================================================
# GÜVENLİK VE ŞİFRE DOĞRULAMA (SECRETS + HASHING)
# ==============================================================================
STORED_SALT = b'\x12\xa4\x88\x91\xc4\xe2\n\x89'
STORED_HASH = b'\xa5\xb1\xcd\x83\xa1\x18\xef\xca\xae\x22\x04\x51\x18\x79\xa1\x02\x95\x6d\xbb\x88\x32\x10\x61\x78\xdf\x98\x10\x12\x87\x32\x8a\x11'

def verify_password(password_input: str) -> bool:
if "ADMIN_PASSWORD" in st.secrets:
return hmac.compare_digest(password_input, st.secrets["ADMIN_PASSWORD"])
input_hash = hashlib.pbkdf2_hmac('sha256', password_input.encode('utf-8'), STORED_SALT, 100000)
...
# ==============================================================================
# VERİTABANI İŞLEMLERİ (REZERVASYON + MANUEL KONTENJAN)
# ==============================================================================
def init_db():
...
st.info(f"💡 **Günlük Kontenjanlar:** Atatürk Havalimanı ({current_limits['Atatürk Havalimanı']} Kişi) | Libadiye Teknoloji Ofisi ({current_limits['Libadiye Teknoloji Ofisi']} Kişi)")
...
kalan_ataturk = max(0, current_limits["Atatürk Havalimanı"] - dolu_ataturk)
...
kalan_libadiye = max(0, current_limits["Libadiye Teknoloji Ofisi"] - dolu_libadiye)
...
if verify_password(girilen_sifre):
...
with col_k1:
yeni_ataturk = st.number_input("Atatürk Havalimanı Kontenjanı", min_value=1, value=limittler["Atatürk Havalimanı"], step=1)
with col_k2:
yeni_libadiye = st.number_input("Libadiye Teknoloji Ofisi Kontenjanı", min_value=1, value=limittler["Libadiye Teknoloji Ofisi"], step=1)
...
"Hafta": hafta,
"Tarih": g,
f"Atatürk (Dolu/{limittler['Atatürk Havalimanı']})": f"{d_ataturk} / {limittler['Atatürk Havalimanı']}",
f"Libadiye (Dolu/{limittler['Libadiye Teknoloji Ofisi']})": f"{d_libadiye} / {limittler['Libadiye Teknoloji Ofisi']}"
})
