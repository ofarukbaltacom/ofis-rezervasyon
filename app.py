...
if not selections:
return JSONResponse(
status_code=400, content={"message": "Lütfen en az bir ofis dan seçiniz!"}
)
...
# Satırları tek tek tarayarak en esnek şekilde veri eşlemesi yapıyoruz
...
# İlk satır başlık ise (içinde 'sicil', 'pnr', 'tarih' gibi kelimeler geçiyorsa) atla
joined_row_str = " ".join(row_vals).lower()
if (
"sicil" in joined_row_str
and "ad" in joined_row_str
and "tarih" in joined_row_str
):
continue
...
# Hücreleri içeriğine göre akıllıca kategorize edelim
for val in row_vals:
if not val:
continue
val_lower = val.lower()

# Tarih tespiti (Örn: 2026-09-01 veya 01.09.2026 veya 1 Eylül)
if (
not res_date
and (
"2026" in val
...
if "eylül" in val_lower:
# Örn: "1 Eylül 2026" veya benzeri
for d_str in [
f"2026-09-{str(i).zfill(2)}" for i in range(1, 31)
]:
day_num = d_str.split("-")[2]
if (
day_num in val
or str(int(day_num)) + " eylül" in val_lower
or str(int(day_num)) + "." in val_lower
):
res_date = d_str
break
if not res_date:
...
res_date = f"2026-{parts[1].zfill(2)}-{parts[0].zfill(2)}"
...
or "müdürlüğü" in val_lower
):
if (
"müdürlüğü" in val_lower
or "anlaşmaları" in val_lower
or "performans" in val_lower
or "kontrol" in val_lower
or "güvenlik" in val_lower
or "gelişim" in val_lower
or "kaynakları" in val_lower
or "fiyatlandırma" in val_lower
or "planlama" in val_lower
or "müsteriler" in val_lower
or "satış" in val_lower
or "iletişimi" in val_lower
):
mudurluk = val
else:
baskanlik = val

# Sicil Tespiti (Genellikle 4-7 haneli sayılar veya TK ile başlar)
if (
not sicil
and val.isdigit()
and 4 <= len(val) <= 8
...
# İsim Tespiti (Rakam içermeyen, uzunluğu uygun metinler)
if (
not name
and not val.isdigit()
and len(val) > 2
and "@" not in val
and not "başkanlık" in val_lower
and not "müdürlüğü" in val_lower
and not "libadiye" in val_lower
and not "atatürk" in val_lower
and not "-" in val
):
name = val

# Eğer hücre bazlı tam yakalanamadıysa indis sırasına göre fallback yap
if not sicil and len(row_vals) >= 2:
for v in row_vals:
...
if not res_date:
res_date = "2026-09-01" # Varsayılan eylül günü
if not location:
location = "Atatürk Havalimanı"
if not baskanlik:
...
content={
"message": (
"Excel dosyası işlenirken hata oluştu. Detay:"
f" {str(e)[:100]}"
)
},
)
...
<p class="text-[11px] text-slate-500">* Yüklediğiniz Excel dosyasındaki tüm satırlar hücre içerikleri taranarak akıllı eşleştirme ile içeri aktarılacak ve kontenjanlardan anında düşülecektir.</p>
</div>
