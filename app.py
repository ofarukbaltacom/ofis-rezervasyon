...
def gun_tesis_dolu_sayisi(tarih, tesis_adi):
   return sum(1 for r in st.session_state.rezervasyonlar if r.get("Tarih") == tarih and r.get("Tesis") == tesis_adi)
# --- SIDEBAR MENÜ ---
...
if sayfa == "📝 Eylül Ayı Rezervasyon Formu":
   ...
   with st.form("aylik_rezervasyon_formu", clear_on_submit=False):
   ...
   if submit_btn:
       ...
       if not (sicil and ad_soyad and mudurluk and unvan):
           st.error("⚠️ Lütfen formdaki tüm kişisel bilgileri eksiksiz doldurunuz!")
       elif not secimler:
           ...
           for hafta, secilen_gunler in secimler.items():
               if len(secilen_gunler) > 2:
                   st.error(f"❌ **{hafta}** için 2 günden fazla seçim yaptınız! Lütfen bu hafta için en fazla 2 gün seçiniz.")
                   kural_ihlali = True
           if not kural_ihlali:
               yeni_kayitlar = []
               ...
               # Hafızaya ve Kalıcı CSV dosyasına ekle
               st.session_state.rezervasyonlar.extend(yeni_kayitlar)
               verileri_toplu_kaydet(yeni_kayitlar)
               st.balloons()
               st.success(f"✅ Sayın **{ad_soyad}**, Eylül ayı için toplam **{len(yeni_kayitlar)} günlük** rezervasyon kaydınız başarıyla tamamlandı!")
               st.rerun()
# ==============================================================================
...
# ==============================================================================
   ...
   if not st.session_state.admin_logged_in:
       ...
       if st.session_state.rezervasyonlar:
           ...
           st.info("Henüz Eylül ayı için kayıtlı bir rezervasyon bulunmamaktadır.")